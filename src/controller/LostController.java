...
public class LostController{
	...
	protected void choice_action(ActionEvent event) {
		...
		data.clear();
		if(all) {
			LostAndFound lost1 = new LostAndFound();
			lost1.setUsername(Parems.getUsername());
			lost1.setLandf("0");
			LostAndFoundService lostService1 = new LostAndFoundService();
			List<LostAndFound> list1 = lostService1.show(lost1);
			...
			all=false;
			my_lost_thing.setText("我的失物");
			return;
		}else {
			all=true;
			my_lost_thing.setText("所有失物");
		}
		for (int i=0;i<list.size();i++) {
			Button btn = new Button();
			btn.setText("删除");
